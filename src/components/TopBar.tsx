  ...
  return <div ref={rootRef} className="relative z-40 px-2 pt-2 sm:px-4 sm:pt-3">
    <header className="shell-panel flex min-h-14 items-center gap-3 rounded-2xl px-3 py-2 sm:px-4">
      ...
      <div className="ml-auto flex gap-2">
        ...
        <button type="button" className="shell-icon-button" data-active={panel === 'usage'} onClick={() => toggle('usage')}><BarChart3 size={14} /><span className="hidden sm:inline">Usage</span>{cost && <span className="hidden text-xs lg:inline">{cost}</span>}</button>
        <button type="button" aria-label="Settings" className="shell-icon-button" onClick={onSettings}><Settings size={14} /></button>
      </div>
    </header>
