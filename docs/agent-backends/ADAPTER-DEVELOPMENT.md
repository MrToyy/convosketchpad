# Agent Backend Adapter 开发规范

本规范是新增或修改 Agent Backend 的必读入口。Canvas 通用层只依赖 `server/lib/agent-backends/contract.ts`；任何原生协议、凭据、进程、Session/Thread、Run/Turn 或工具事件差异都必须封装在对应 Adapter 目录内。

## 目录与依赖方向

```text
server/lib/agent-backends/
  contract.ts                 # 唯一上层 Port、事件、Capability 与 Handle 类型
  config.ts                   # 已配置 Backend 类型；同一类型最多一个实例
  registry.ts                 # 生命周期与按 backendId 查找
  catalog.ts                  # Agent、状态的跨 Backend 聚合
  adapters/
    <backend-id>/
...
依赖只能从 Canvas/Route/Coordinator 指向 `contract`、`registry`、`catalog`。通用代码不得导入 `adapters/*`，不得检查 `backendId === "openclaw"` 后调用原生能力，也不得解析 Handle 的 `opaque`。只有 `registry.ts` 负责组合具体 Adapter。

## 注册与 Agent 目录

- `AGENT_BACKENDS` 是逗号分隔的 Backend 类型列表；未知值、重复配置语义或空列表必须在启动时明确失败。
- 当前模型中同一 Adapter 类型最多一个实例，因此稳定 `backendId` 就是类型 ID。未来若需要多实例，必须先扩展配置模型，不能悄悄复用同一 ID。
- 每个 Adapter 返回自己的 Agent Profile；`catalog.ts` 按配置顺序、各 Backend 默认 Profile、原生顺序形成一个扁平目录。
- 所有 Profile 在产品中平权。新建 Canvas 不弹窗，自动绑定目录中的第一个可用 Profile；首次持久发送预留前可以修改，预留创建后永久锁定。
- Backend 断开时可展示最后一次成功发现的 Profile，但必须标为不可用，不能用陈旧目录执行发送。

## Port 与不透明 Handle

Adapter 必须实现 `AgentBackend` 的完整 Port。Conversation、Turn、Artifact、Approval 使用：
...
所有原生事件先归一化为 `BackendEvent`。终态、审批 required/resolved 和需要恢复的断线事实进入 `backend_event_inbox` 去重；流式文本只做瞬时 Preview。
...
- `accepted`：Backend 已确认，节点状态转为 resolved/denied；
...
- `/api/runtime/status` 返回每个 Backend 状态以及 `ready | degraded | connecting | unavailable` 总态；部分 Backend 失败不得隐藏健康 Backend。
- Agent 目录与状态可并行读取；单个 Backend 失败必须局部降级。
- `/api/runtime/usage` 保留每个 Backend 的用量和 Provider 额度分组。Token、额度窗口、Provider 名称不可跨 Backend 相加。
- 只有所有已配置 Backend 均可连接、所有声明支持账户费用的 Backend 都成功返回 `additive=true` 的费用，且币种和统计周期一致时，服务端才返回 `comparableCostTotal`；缺失数据不能伪装成全局合计。
- Branch 数、工作中数量和上下文 Meter 属于当前 Canvas，不参与账户级聚合。

## 数据库与迁移

通用表只使用 `backend_id`、`agent_profile_id`、`conversation_*`、`backend_turn_id`、`turn_ref_json`、`backend_artifact_*`、`execution_metadata_json` 和 `backend_event_inbox`。不得为新 Adapter 增加顶层协议字段或双写别名。
