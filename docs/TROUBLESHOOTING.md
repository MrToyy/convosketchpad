...
如果 `--status` 显示 `in-progress` 或 `failed` 的活动事务，不要删除 `active-transaction.json`、维护 Lease 或快照，也不要直接再次运行普通 update。使用 `npm run update -- --resume`：它先按事务保存的准确快照恢复代码、环境和可用的完整数据库，恢复升级前服务状态，再重新执行原目标 Release。快照不完整、Hash 不匹配、服务归属丢失或服务状态发生意外变化时，resume 会保持现状并失败关闭。

如果项目 `.env` 自定义了 `CONVOSKETCHPAD_DATA_DIR`，但当前 Shell 没有导出它，应把上面的路径替换为 `.env` 中的实际值。系统级 systemd 更新在交互终端会请求 sudo；CI、SSH 无终端任务或其他非交互执行必须事先提供无提示授权，否则 `sudo -n` 会让更新明确失败。不要在更新器失败后再次运行安装器跨版本覆盖；没有活动事务时，排除权限或构建问题后重试 `npm run update`，或用 `npm run update -- --rollback` 恢复正式完整 `last-good` 快照。
...
官方 `0.4.1` 升级到 `0.4.2` 时，目标迁移器会验证旧 PID 锁属于自身直接父进程。若这一跳仍报告 legacy maintenance handoff 无效或 maintenance operation 正在运行，不要手工删除锁或伪造环境变量；先确认安装目录、`.env` 中的 `CONVOSKETCHPAD_DATA_DIR` 和实际执行更新的父进程一致。该错误表示安全交接未成立，数据库不会因旧离线变量而被打开。
