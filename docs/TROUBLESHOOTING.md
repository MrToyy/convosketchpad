...
Common issues and solutions for ConvoSketchpad.
...
3. Token mismatch between ConvoSketchpad server config and gateway
...
- For trusted official-gateway access, `/api/connect-defaults` advertises `serverSideAuth=true` and ConvoSketchpad connects with an empty browser-side token
...
**Cause:** WebSocket connection to gateway dropped. ConvoSketchpad auto-reconnects with exponential backoff (1s base, 30s max).
...
# Check ConvoSketchpad health (includes gateway probe)
...
# Restart ConvoSketchpad — a new keypair will be generated
```
...
**Cause:** Those panels can use the server-side gateway RPC fallback, which opens its own WebSocket to the gateway. If ConvoSketchpad does not know its real browser-facing origin, that fallback can present the wrong origin even though the browser chat path is already working.
...
Also add `https://nerve.example.com` to `gateway.controlUi.allowedOrigins`, then restart both ConvoSketchpad and the gateway.
...
2. **Token mismatch across config files.** OpenClaw 2026.2.19 has a known bug where `openclaw onboard` writes different tokens to the systemd service file and `openclaw.json`. The gateway uses the systemd env var; ConvoSketchpad reads from `.env`.
...
grep GATEWAY_TOKEN .env                                 # ConvoSketchpad config
```
...
# Approve the ConvoSketchpad device
...
If ConvoSketchpad and the gateway are on the same machine, re-running `npm run setup` can patch it for you.
...
**Fix:** Restart the ConvoSketchpad server to clear the in-memory TTS cache.
...
- If accessing remotely, use HTTPS:
  ...
  openssl req -x509 -newkey rsa:2048 -nodes \
    ...
    -subj "/CN=localhost"
  # ConvoSketchpad auto-detects certs and starts HTTPS on port 3443
  ```
...
- Check that ConvoSketchpad can read the active OpenClaw config file
...
If you call the HTTP fallback with only `thinkingLevel`, it returns **501**. If ConvoSketchpad cannot find an active root session and you omitted `sessionKey`, it can return **409**.
