...
server/lib/agent-runtimes/
  ...
  definitions.ts              # 清单对应的实例、配置解析与校验
...
依赖只能从 Canvas/Route/Coordinator 指向 `contract`、`registry`、`catalog`。通用代码不得导入 `adapters/*`，不得检查 `runtimeId === "openclaw"` 后调用原生能力，也不得解析 Handle 的 `opaque`。只有 `definitions.ts` 导入具体 Adapter，`registry.ts` 只消费 Definition。ESLint 对全部 Canvas 业务模块和 Route 使用目录模式阻止具体 Adapter 导入，新增文件不需要手工加入白名单。

## 注册与 Agent 目录

- `AGENT_RUNTIMES` 是逗号分隔的 Runtime 类型列表；未知值、重复配置语义或空列表必须在启动时明确失败。
- 新 Adapter 必须同时向纯数据 `manifest.ts` 增加 ID/展示名、向 `definitions.ts` 增加由 TypeScript 完整性检查约束的唯一 Definition，并在 setup registry 注册探测/配置驱动；不得另外维护支持数组、注册链式 `if/else` 或把专属环境变量加回通用 `server/lib/config.ts`。
...
- 新的 Runtime 环境变量要进入 `.env.example`、env writer、配置校验和更新器快照/回滚覆盖；Driver 通过通用 `args.options` 读取自身 CLI 选项，不得在 `RuntimeSetupArguments` 增加某个协议专属字段。仅增加 Runtime 或默认 Agent 配置通常不需要数据库迁移。
