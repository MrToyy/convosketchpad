# Add Tailscale to an Existing ConvoSketchpad Install

This guide is for the case where **ConvoSketchpad is already installed and working**, and you want to add private remote access afterward.

Use one of these two paths:

- **Tailnet IP**: quickest path, ConvoSketchpad listens on the Tailscale IP and you open `http://100.x.y.z:3080`
- **Tailscale Serve**: better default for phones and voice input, ConvoSketchpad stays on `127.0.0.1` and Tailscale exposes `https://<node>.tail<id>.ts.net`
...
- ConvoSketchpad starts locally and `curl http://127.0.0.1:3080/health` works
- OpenClaw gateway is healthy and `openclaw gateway status` works
- Tailscale is installed on the ConvoSketchpad machine
- Tailscale is logged in on the ConvoSketchpad machine and on the client device you want to use
- You know where your ConvoSketchpad install lives, default is usually `~/nerve`
...
- you are okay with ConvoSketchpad binding to `0.0.0.0`

Choose **Tailscale Serve** if:
- you want ConvoSketchpad to stay private on localhost
- you want an HTTPS URL for phone access
- you want the least surprising path for microphone access on mobile browsers

## Option A: Tailnet IP

This exposes ConvoSketchpad on the machine's Tailscale IP and patches both ConvoSketchpad and the gateway to allow that origin.
...
- `NERVE_AUTH=true` is strongly recommended whenever ConvoSketchpad is reachable over the network, including Tailscale
- if you do not already have a password hash configured, ConvoSketchpad accepts the `GATEWAY_TOKEN` as a fallback password
...
### 4. Restart ConvoSketchpad and the gateway
...
This keeps ConvoSketchpad on localhost and lets Tailscale publish a private HTTPS URL.
...
Notes:
- if `HOST` is missing entirely, ConvoSketchpad defaults to localhost, which is also fine
...
### 5. Restart ConvoSketchpad and the gateway
...
- phone access works without exposing ConvoSketchpad directly on `0.0.0.0`
...
- Do **not** expose OpenClaw gateway port `18789` publicly just because ConvoSketchpad is on Tailscale
