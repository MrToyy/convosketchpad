...
此目录是 ConvoSketchpad 的 Canvas 与受管用户认证代码导航层。目标是先根据任务定位到一份短文档，再从该文档直接进入代码、测试或详细设计，避免每次重新搜索整个继承自 Nerve 的代码库。
...
| 同步 Nerve upstream、维护 `master`、合并到 `main`、提交或推送 | [Git 工作流](./GIT-WORKFLOW.md) |
