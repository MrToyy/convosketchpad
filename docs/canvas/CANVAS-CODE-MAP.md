...
| Agent Runtime 契约、Definition、Registry、聚合目录与开发规范 | `server/lib/agent-runtimes/contract.ts`、`definitions.ts`、`registry.ts`、`catalog.ts`、`docs/agent-runtimes/ADAPTER-DEVELOPMENT.md` |
...
发送分层测试位于 `server/lib/canvas-domain.test.ts`、`server/lib/canvas-send-service.test.ts`、`server/lib/canvas-public-dto.test.ts` 与 `server/lib/agent-runtimes/adapters/openclaw/*.test.ts`；Registry/聚合入口由 `server/lib/agent-runtimes/registry.test.ts` 覆盖。前端审批、控制与投影由 `src/features/canvas/CanvasNodes.resize.test.tsx`、`src/features/canvas/useCanvasComposerDrafts.test.tsx`、`src/features/canvas/canvas-flow-projection.test.ts` 和 Canvas/SSE 测试覆盖。数据库测试从完整 `0.2.0` fixture 建库，验证旧字段物理移除、Handle 回填、审批、事件去重、状态转换、Artifact、附件及重启幂等。
