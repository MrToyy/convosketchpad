...
ConvoSketchpad is configured via a `.env` file in the project root. All variables have sensible defaults — only `GATEWAY_TOKEN` is strictly required.

---

## Setup Wizard

The interactive setup wizard is the recommended way to configure ConvoSketchpad:
...
Connects ConvoSketchpad to your OpenClaw gateway. The wizard auto-detects the gateway token from:
...
- Pre-pairs ConvoSketchpad's device identity in the normal setup path so it can connect without manual approval (`openclaw devices approve`)
...
Determines how you'll access ConvoSketchpad. The wizard auto-configures `HOST`, `ALLOWED_ORIGINS`, `WS_ALLOWED_HOSTS`, and `CSP_CONNECT_EXTRA` based on your choice:
...
| **Tailscale Serve** | `127.0.0.1` | Keeps ConvoSketchpad loopback-only and exposes it through a Tailscale Serve HTTPS hostname when available. |
...
If you choose a network-exposed mode, the wizard prompts you to enable managed-user Token auth. After setup, an administrator creates users with `npm run users -- add`; Canvas data is scoped to that identity.
...
> **⚠️ Network exposure:** Setting `HOST=0.0.0.0` exposes all endpoints to the network. Enable trusted-user Token authentication (`NERVE_AUTH=true`) before binding to a non-loopback address. This MVP authentication is intended for controlled environments; see [Security](SECURITY.md).
...
| `NERVE_PUBLIC_ORIGIN` | *(empty)* | No | Explicit browser-facing ConvoSketchpad origin used when server-side gateway RPC fallback must open its own WebSocket to OpenClaw. Useful for reverse-proxy, cloud, and hybrid deployments. |
...
ConvoSketchpad performs **server-side token injection**. When a connection is established through the WebSocket proxy, ConvoSketchpad automatically injects the configured `GATEWAY_TOKEN` into the connection request if the client is considered **trusted**.

**Trust is granted if:**
1. The connection is from a **local loopback address** (`127.0.0.1` or `::1`). When ConvoSketchpad is behind a trusted reverse proxy, proxy-aware client IP handling can preserve that loopback detection (see `TRUSTED_PROXIES`).
...
ConvoSketchpad uses explicit language selection (`NERVE_LANGUAGE`) for voice flows; there is no user-facing auto-detect language mode.
...
If you are retrofitting Tailscale onto an existing install, see [Add Tailscale to an Existing ConvoSketchpad Install](TAILSCALE.md).

### Authentication

ConvoSketchpad includes a built-in authentication layer that protects all API endpoints, WebSocket connections, and SSE streams with a session cookie. Auth is opt-in for localhost users and auto-prompted during setup when binding to a network interface.

| Variable | Default | Description |
|----------|---------|-------------|
| `NERVE_AUTH` | `false` | Enable trusted-user Token authentication and Canvas owner isolation |
| `NERVE_PASSWORD_HASH` | *(deprecated)* | Ignored by the Canvas MVP token login |
...
| `NERVE_AUTH_MAX_FAILURES` | `3` | Failed token verifications allowed per client IP before lockout |
| `NERVE_AUTH_FAILURE_WINDOW` | `1800000` | Failure counting window in milliseconds |
| `NERVE_AUTH_LOCKOUT` | `1800000` | Lockout duration in milliseconds; stored in memory and cleared by restart |
...
- Only tokens created by the local `npm run users` command can log in; unknown tokens are rejected
- Three failed token verifications from one client IP within 30 minutes lock that IP for 30 minutes

**User management:**

```bash
npm run users -- add Alice --token example-token
npm run users -- add Bob                   # generates and prints a random token once
npm run users -- list
npm run users -- rotate Alice              # revokes old sessions and generates a new token
npm run users -- disable Alice
npm run users -- enable Alice
```

The first managed user automatically inherits all Canvas data owned by the unauthenticated `Local User`. Users are never created by the login endpoint, and there is intentionally no user deletion command.

**Setup wizard:** When the access mode is non-localhost, the wizard enables trusted-user token auth and generates the session secret.
...
ConvoSketchpad automatically starts an HTTPS server on `SSL_PORT` when certificates exist at:
...
NERVE_AUTH_MAX_FAILURES=3
NERVE_AUTH_FAILURE_WINDOW=1800000
NERVE_AUTH_LOCKOUT=1800000
