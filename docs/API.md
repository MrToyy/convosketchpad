...
ConvoSketchpad exposes a REST + SSE API served by [Hono](https://hono.dev/) on the configured `PORT` (default **3080**). All API routes are prefixed with `/api/` except the health endpoint. Responses are JSON unless otherwise noted.
...
Provides the official gateway WebSocket URL and trust metadata for the frontend's auto-connect flow. The `token` field is always `null`; when `serverSideAuth` is `true`, ConvoSketchpad expects the browser to connect with an empty token and injects `GATEWAY_TOKEN` server-side during the WebSocket handshake.
...
`serverSideAuth` becomes `true` when ConvoSketchpad can safely inject the configured gateway token for this request, such as:
- loopback / tunneled local access to the official gateway URL
- authenticated sessions on a network-exposed ConvoSketchpad instance
...
All memory routes accept an optional `agentId` scope. If omitted, ConvoSketchpad uses `main`. The UI normally derives this from the owning top-level agent when you switch sessions.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `sessionKey` | `string` | No | Target session. If omitted, ConvoSketchpad tries to pick a preferred active root session |
...
Workspace file routes accept an optional `agentId` scope. If omitted, ConvoSketchpad uses the `main` workspace.
...
ConvoSketchpad scopes this by creating a temporary OpenClaw config whose `agents.defaults.workspace` points at the selected agent workspace, then runs the CLI against that workspace.
...
All file-browser routes accept an optional `agentId` scope. If omitted, ConvoSketchpad uses `main`. If `FILE_BROWSER_ROOT` is set, file-browser agent scoping is bypassed and all sessions browse the same custom root.
...
**Execution paths:**
- **Assigned tasks** create a real child session beneath the assignee's live root. ConvoSketchpad verifies that the parent root exists, creates the child with `sessions.create(parentSessionKey=...)`, then sends the task into that child with `sessions.send`.
...
- When an assigned child session finishes or fails, ConvoSketchpad sends a completion report back to the parent root session.
