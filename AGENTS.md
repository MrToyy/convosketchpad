...
本仓库是独立维护的 ConvoSketchpad，包含 Canvas 与受管用户认证功能。开始工作前，先按任务类型读取对应索引；不要依靠全仓搜索重新推断已经记录的结构。
...
| 登录、用户 Token、用户隔离、限流、Cookie、WS 鉴权 | [`docs/canvas/AUTH-CODE-MAP.md`](docs/canvas/AUTH-CODE-MAP.md) | Auth 组件、CLI、SQLite 用户数据与会话撤销链路 |
...
- 产品目标与原则：[`docs/PRODUCT.md`](docs/PRODUCT.md)
- 架构、Session 和数据流：[`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)
...
- 改变 Branch、Interaction、附件、Artifact、用户或会话语义时，同时更新架构文档。
- 改变产品目标或核心体验时，更新产品文档。
