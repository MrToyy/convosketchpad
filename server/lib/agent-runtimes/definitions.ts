...
export interface AgentRuntimeDefinition {
  ...
  validateConfig(): { warnings: string[]; errors: string[] };
}
...
export const agentRuntimeDefinitions = {
  openclaw: {
    ...
    validateConfig: validateOpenClawConfig,
  },
} satisfies Record<SupportedAgentRuntimeId, AgentRuntimeDefinition>;

export type { SupportedAgentRuntimeId } from './manifest.js';
...
export function configuredAgentRuntimeIds(
  ...
  const unsupported = unique.filter((entry) => !(entry in agentRuntimeDefinitions));
...
export function validateConfiguredAgentRuntimes(
  ...
  for (const id of ids) {
    const result = agentRuntimeDefinitions[id].validateConfig();
