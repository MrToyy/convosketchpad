...
export function buildCanvasReplayPlan(
  ...
  const reasonText = reason === 'canonical-replay'
    ...
    : 'OpenClaw replaced or reset this Canvas session. Restore this complete immutable branch context before continuing.';
  const resources = [...physicalByIdentity.values()];
  const resourceNote = resources.length > 0
    ? '\n\nRestored files are attached through OpenClaw in F001, F002, ... order. F references in the history identify their original interaction and role.'
    : '';
  return {
