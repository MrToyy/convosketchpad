 ...
 */
export function createAuthRoutes(store?: CanvasStore) {
  const app = new Hono();
  ...
  app.post('/api/auth/login', rateLimitAuth, async (c) => {
    ...
    try {
      ...
      const identity = await authenticateManagedToken(token, store);
      if (!identity) {
 ...
 */
  app.get('/api/auth/status', (c) => {
    ...
    const identity = resolveManagedSession(session, store);
